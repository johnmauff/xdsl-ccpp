...
| `kessler/` | Kessler microphysics scheme; three drivers: CCPP Fortran cap, hand-written Fortran, and C++ BIND(C); OpenACC GPU directives (`memory_space = device`) |
...
| Multi-language host model path | ❌ | ❌ | ❌ | ✅ Kessler C++ driver (see below) |
...
- **kessler**: Kessler warm-rain microphysics scheme with OpenACC GPU directives generated by xdsl-ccpp; CPU execution verified bit-for-bit against the hand-written original cap using gfortran; GPU execution not yet tested. Three drivers are provided: `kessler_ccpp` (CCPP Fortran cap), `kessler_hand` (hand-written Fortran), and `kessler_cxx` (C++ calling BIND(C) CCPP caps); all three produce bit-for-bit identical results‡
...
‡ The C++ BIND(C) driver (`kessler_cxx`) calls the CCPP-generated BIND(C) Fortran cap from C++. All three drivers produce identical numerical output, confirming the multi-language cap path is correct end-to-end.
...
correctness is verified across all five example test cases — helloworld, capgen,
ddthost, advection, and kessler — including a C++ BIND(C) driver that calls
CCPP-generated Fortran caps directly from C++ and produces bit-for-bit identical
results alongside the Fortran drivers. Unit conversion uses local temporaries (consistent with
...
| C++ host model calling Fortran physics schemes | xdsl-ccpp |
