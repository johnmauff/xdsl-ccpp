...
- **#49 (constituent_cap patterns) -- scoped, not yet implemented:** signature-splicing is already unified for 4 of 8 subroutines via existing `_instance_arg`/`_instance_decl` helpers; only `ca_lines`/`mp_lines`'s tiny duplicated ternary is unaddressed. Precondition-guards are actually 4 distinct semantics (silent-skip, bare-return, **lazy-allocate** -- opposite polarity from the others, bare-return, full-error-return) -- only the 4 full-error-return sites in `ic_lines`/`ci_lines` are safe to merge into one `_error_guard()`; the rest must stay separate.
- **#51 (`cpp_interop.py` arg-info schema) -- scoped, not yet implemented:** 3 construction sites with genuinely different key sets (not copies) -- the DDT-member site adds 5 keys and computes `rank` on a different basis ("usually agreeing by convention, not proven equivalent"). Needs a real design decision (an all-optional superset dataclass, and reconciling the `rank` discrepancy) before touching ~3 construction + ~10 read sites -- the largest remaining lift in this batch.
- **#52 (small cleanups A) -- partially scoped:** Item A (`host_var_match_pass.py` dim-split, 2 identical sites) is clean and ready. Item B (`gpu_ccpp_cap_pass.py` suite-arg iterator) turned out to be the same pattern as task #43's own "Shape B" (~25+ sites) -- recommend folding into #43's eventual fix rather than a bespoke helper.
- **#55 (`ccpp_cap.py` dict consolidation) -- scoped, not yet implemented:** the 4 top-level dicts `_build_cap_var_map` returns are NOT redundant (different consumers). The real opportunity is one level down: 5 parallel per-arg dicts inside the per-group loop, all keyed by the same bare name, built in one pass -- worth merging into one dict-of-namedtuple. The "lc_instances lazy-alloc duplication" half of the original title doesn't hold up: there is exactly one allocation site in the whole codebase (`constituent_cap.py`'s `rc_lines`), already covered by task #49's guard-check consolidation -- recommend narrowing #55 to just the dict merge.
- **Tier 3 — structural decomposition, deliberately deferred** (tasks #56-#60): `suite_cap.py`'s `GenerateSuiteSubroutine` is a 2,929-line, ~50-method god-class with no single responsibility; `run_dispatch.py`'s `_build_run_dispatch_chain` is an ~800-line monolith breaking the file's own otherwise-good pattern of small dataclass-returning helpers; `print_ftn.py`'s `_print_fn` and `ccpp_cap.py`'s `_build_suite_variables_fn` each do 5-6 distinct jobs in one function; `suite_cap.py`'s kind/unit-cast construction and `_build_block_signature` have their own decomposition opportunities (the latter tied directly to task #30's existing chained-interstitial bug — fix together); and an open design question (not a mechanical fix) about whether `lifecycle_cap.py`'s inline arg-resolution dispatch should be unified with `run_dispatch.py`'s cleaner `ResolvedArgOp`/`ArgSourceKind` pattern. These touch the highest-traffic, most heavily-used files in the pipeline and need careful staged review with full regeneration/test verification at each step — the same discipline this engagement already used for the vocabulary-resolution redesign (Stages 1-4) and the multi-instance migration (5 stages). Not part of the 2026-08-18 execution pass; queued for a dedicated future session.
