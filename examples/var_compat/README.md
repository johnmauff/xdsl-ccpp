...
- **Not yet fixed — the dynamic-count subcycle's loop bound
  (`num_subcycles_for_effr`) is emitted as the literal standard_name string
  instead of being resolved to the host's own local name
  (`num_subcycles`).** A real `xdsl_ccpp` code gap in `suite_cap.py`'s
  subcycle-loop-count handling — no other example in the repo uses a named,
  non-literal subcycle loop count, so this path was never exercised before.
  See `ccpp_cap_refactor_plan.md`'s backlog.
- **Not yet fixed — the host-facing wrapper subroutine (generated by
  `ccpp_cap.py`, a separate code path from the suite-cap subroutine the
  fixes above live in) declares `scalar_var`/`tke_inout` `intent(in)`,
  while the suite-cap subroutine it calls now correctly declares them
  `intent(inout)`** — passing an `intent(in)` actual argument into an
  `intent(inout)` dummy argument is invalid Fortran. Root cause not yet
  located; unrelated to the DDT issue above. See
...
make run    # build and run -- see the known wrapper intent(in)/intent(inout)
            # mismatch and unresolved subcycle-loop-bound issues above; this
            # will not compile until both are fixed
