...
this example's real generated output. One known issue remains, unrelated to
any of the above: the host-facing wrapper subroutine (generated by
`ccpp_cap.py`, a separate code path from the suite-cap subroutine these
fixes live in) declares `scalar_var`/`tke_inout` `intent(in)`, while the
suite-cap subroutine it calls now correctly declares them `intent(inout)` —
passing an `intent(in)` actual argument into an `intent(inout)` dummy
argument is invalid Fortran. See `ccpp_cap_refactor_plan.md`'s backlog for
the writeup; root cause not yet located.
...
make run    # build and run -- see the known wrapper intent(in)/intent(inout)
            # mismatch above; this may not compile until that's fixed
