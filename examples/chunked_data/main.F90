program test_chunked_data
  ...
  ! not chunked, since this bracketing phase isn't split into chunks.
  call ccpp_physics_timestep_init(ccpp_suite, ccpp_group, 1, ncols, &
  ...
  ! matching ccpp_physics_timestep_init's own signature -- full extent
  ! (1, ncols), not chunked.
  call ccpp_physics_timestep_final(ccpp_suite, ccpp_group, 1, ncols, &
      chunked_data_instance%array_data(1:ncols), errmsg, errflg)
  if (errflg/=0) then
