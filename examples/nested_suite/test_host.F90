...
contains
  ...
  subroutine test_host(retval, test_suites)
    ...
    ! group-scoped (task #28, Stage 1) -- called once per suite part, full
    ! column extent (no chunking needed for this phase), matching
    ! ccpp_physics_run's own per-part call below.
    do sind = 1, num_suites
      if (errflg /= 0) then
        exit
      end if
    ...
    ! Finalize the timestep. ccpp_physics_timestep_final is now group-scoped
    ! (task #28, Stage 2) -- called once per suite part, full column extent
    ! (no chunking needed for this phase), matching
    ! ccpp_physics_timestep_init's own per-part call above.
    do sind = 1, num_suites
      if (errflg /= 0) then
        exit
      end if
      ...
      if (errflg == 0) then
        ...
        if (errflg /= 0) then
          write(6, '(3a)') trim(test_suites(sind)%suite_name), ': ', &
              trim(errmsg)
          write(6, '(2a)') 'An error occurred in ccpp_physics_finalize, ', &
              'Exiting...'
          exit
        end if
      end if
