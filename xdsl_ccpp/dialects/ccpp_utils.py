...
class CHostCapOp(IRDLOperation):
    ...
    Holds the complete Fortran module text (``ftn_text``) and matching C++
    header text (``cpp_text``) as pre-built strings.  Generated by the
    ``generate-ccpp-cap`` pass when ``explicit_args=True``; consumed by
    ``print_ftn.py`` (emits ``ftn_text`` verbatim) and ``print_cpp_header.py``
    (emits ``cpp_text`` verbatim).
