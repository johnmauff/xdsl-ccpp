...
// example.  Exercises: five distinct schemes in one suite, a scheme that
// appears twice in the suite XML (apply_constituent_tendencies), host/module
// metadata files, 3-D array arguments, and cld_shadow -- a scheme whose own
// local names ("cld_ice_array", "ncols") reuse names already used elsewhere
// in the group cap for unrelated standard_names (capgen-v1 GitHub issues
// #772/#774), each correctly renamed to avoid a module-scope collision.
//
...
// CHECK-NEXT:    use cld_shadow, only: cld_shadow_run
...
// CHECK:       CONTAINS
// CHECK-LABEL:   subroutine cld_suite_register(dyn_const, dyn_const_ice, errmsg, errflg)
...
// CHECK-LABEL:   subroutine cld_suite_initialize(errflg, errmsg)
...
// CHECK-LABEL:   subroutine cld_suite_finalize(errflg, errmsg)
// CHECK:           integer, intent(out) :: errflg
// CHECK-NEXT:      character(len=512), intent(out) :: errmsg
// CHECK:           errflg = 0
// CHECK-NEXT:      errmsg = ''
...
// CHECK-LABEL:   subroutine cld_suite_init_physics(const_std_name, num_consts, test_stdname_array, const_inds,   &
// CHECK:           tfreeze, const_index, errmsg, errflg)
...
// CHECK-NEXT:      if (.not. allocated(cld_ice_cld_ice_array)) then
// CHECK-NEXT:        allocate(cld_ice_cld_ice_array(ncols, pver))
// CHECK-NEXT:        cld_ice_cld_ice_array = 0.0_kind_phys
// CHECK-NEXT:  #ifdef USE_GPU
// CHECK-NEXT:        !$acc enter data create(cld_ice_cld_ice_array)
// CHECK-NEXT:  #endif
// CHECK-NEXT:      end if
...
// CHECK-LABEL:   subroutine cld_suite_physics(const_std_name, num_consts, test_stdname_array, const_inds, ncol,  &
// CHECK:           timestep, temp, qv, ps, cld_liq_tend, const_tend, const, const_index, errmsg, errflg)
// CHECK-NEXT:      character(len=32), intent(in) :: const_std_name
// CHECK-NEXT:      integer, intent(in) :: num_consts
// CHECK-NEXT:      character(len=32), target, intent(in) :: test_stdname_array(:)
// CHECK-NEXT:      integer, target, intent(inout) :: const_inds(:)
...
// CHECK-NEXT:      character(len=512), intent(out) :: errmsg
// CHECK-NEXT:      integer, intent(out) :: errflg
// CHECK-NEXT:      real(kind=kind_phys), allocatable :: ps_unit_conv(:)
// CHECK:           errflg = 0
// CHECK-NEXT:      errmsg = ''
...
// CHECK-LABEL:   subroutine cld_suite_timestep_final_physics(errflg, errmsg)
// CHECK:           integer, intent(out) :: errflg
// CHECK-NEXT:      character(len=512), intent(out) :: errmsg
// CHECK:           errflg = 0
// CHECK-NEXT:      errmsg = ''
...
// CHECK-LABEL:   subroutine cld_suite_final_physics(errmsg, errflg)
...
// CHECK-NEXT:    use test_host_mod, only: ncnst
// CHECK-NEXT:    use test_host_mod, only: ncols
...
// CHECK-NEXT:        call cld_suite_register(lc_dyn_const, lc_dyn_const_ice, errmsg, errflg)
// CHECK-NEXT:      else
...
// CHECK:           errflg = 0
// CHECK-NEXT:      if (trim(suite_name) .eq. 'cld_suite') then
// CHECK-NEXT:        call cld_suite_initialize(errflg, errmsg)
// CHECK-NEXT:      else
// CHECK-NEXT:        write(errmsg, '(3a)') "No suite named ", trim(suite_name), " found"
// CHECK-NEXT:        errflg = 1
// CHECK-NEXT:      end if
...
// CHECK:           errflg = 0
// CHECK-NEXT:      if (trim(suite_name) .eq. 'cld_suite') then
// CHECK-NEXT:        call cld_suite_finalize(errflg, errmsg)
// CHECK-NEXT:      else
// CHECK-NEXT:        write(errmsg, '(3a)') "No suite named ", trim(suite_name), " found"
// CHECK-NEXT:        errflg = 1
// CHECK-NEXT:      end if
...
// CHECK-NEXT:          call cld_suite_physics(const_std_name, num_consts, std_name_array, const_inds, ncol, dt,  &
// CHECK-NEXT:            phys_state%Temp(col_start:col_end, 1:pver), phys_state%q(col_start:col_end, 1:pver,     &
// CHECK-NEXT:            index_qv), phys_state%ps(col_start:col_end), lc_cld_liq_tend(col_start:col_end,         &
// CHECK-NEXT:            1:pver), lc_const_tend, lc_constituent_array, const_index, errmsg, errflg)
// CHECK-NEXT:        else
...
// CHECK-NEXT:      if (trim(suite_name) .eq. 'cld_suite') then
// CHECK-NEXT:        if (trim(suite_part) .eq. 'physics') then
// CHECK-NEXT:          call cld_suite_timestep_init_physics(errflg, errmsg)
// CHECK-NEXT:        else
// CHECK-NEXT:          write(errmsg, '(3a)') "No suite part named ", trim(suite_part), " found in suite cld_suite"
// CHECK-NEXT:          errflg = 1
// CHECK-NEXT:        end if
...
// CHECK-NEXT:      if (trim(suite_name) .eq. 'cld_suite') then
// CHECK-NEXT:        if (trim(suite_part) .eq. 'physics') then
// CHECK-NEXT:          call cld_suite_timestep_final_physics(errflg, errmsg)
// CHECK-NEXT:        else
// CHECK-NEXT:          write(errmsg, '(3a)') "No suite part named ", trim(suite_part), " found in suite cld_suite"
// CHECK-NEXT:          errflg = 1
// CHECK-NEXT:        end if
...
// CHECK-NEXT:      if (trim(suite_name) .eq. 'cld_suite') then
// CHECK-NEXT:        if (trim(suite_part) .eq. 'physics') then
// CHECK-NEXT:          call cld_suite_init_physics(const_std_name, num_consts, std_name_array, const_inds,       &
...
// CHECK-NEXT:      if (trim(suite_name) .eq. 'cld_suite') then
// CHECK-NEXT:        if (trim(suite_part) .eq. 'physics') then
// CHECK-NEXT:          call cld_suite_final_physics(errmsg, errflg)
// CHECK-NEXT:        else
// CHECK-NEXT:          write(errmsg, '(3a)') "No suite part named ", trim(suite_part), " found in suite cld_suite"
// CHECK-NEXT:          errflg = 1
// CHECK-NEXT:        end if
...
// CHECK-NEXT:          var_list(6) = 'number_of_ccpp_constituents         '
// CHECK-NEXT:          var_list(7) = 'surface_air_pressure                '
// CHECK-NEXT:          var_list(8) = 'temperature                         '
// CHECK-NEXT:          var_list(9) = 'tendency_of_cloud_liquid_dry_mixing_ratio'
// CHECK-NEXT:          var_list(10) = 'time_step_for_physics               '
// CHECK-NEXT:          var_list(11) = 'water_temperature_at_freezing       '
...
// CHECK-NEXT:          var_list(11) = 'test_banana_constituent_index       '
// CHECK-NEXT:          var_list(12) = 'test_banana_constituent_indices     '
...
// CHECK-NEXT:          var_list(10) = 'number_of_ccpp_constituents         '
// CHECK-NEXT:          var_list(11) = 'surface_air_pressure                '
// CHECK-NEXT:          var_list(12) = 'temperature                         '
// CHECK-NEXT:          var_list(13) = 'tendency_of_cloud_liquid_dry_mixing_ratio'
// CHECK-NEXT:          var_list(14) = 'test_banana_constituent_index       '
// CHECK-NEXT:          var_list(15) = 'test_banana_constituent_indices     '
// CHECK-NEXT:          var_list(16) = 'time_step_for_physics               '
// CHECK-NEXT:          var_list(17) = 'water_temperature_at_freezing       '
// CHECK-NEXT:          var_list(18) = 'water_vapor_specific_humidity       '
// CHECK-NEXT:        end if
// CHECK-NEXT:      else
// CHECK-NEXT:        write(errmsg, '(3a)') "No suite named ", trim(suite_name), " found"
// CHECK-NEXT:        errflg = 1
...
// CHECK-NEXT:          call lc_dyn_const(lc_i)%standard_name(lc_src_std_name)
// CHECK-NEXT:          call lc_dyn_const(lc_i)%units(lc_src_units)
...
// CHECK-NEXT:          call lc_dyn_const_ice(lc_i)%standard_name(lc_src_std_name)
// CHECK-NEXT:          call lc_dyn_const_ice(lc_i)%units(lc_src_units)
// CHECK-NEXT:          lc_found = .false.
// CHECK-NEXT:          do lc_j = 1, lc_num
// CHECK-NEXT:            call lc_tmp(lc_j)%standard_name(lc_dst_std_name)
// CHECK-NEXT:            if (trim(lc_dst_std_name) == trim(lc_src_std_name)) then
...
// CHECK-NEXT:      do lc_j = 1, lc_num
// CHECK-NEXT:        call lc_tmp(lc_j)%standard_name(lc_dst_std_name)
// CHECK-NEXT:        if (trim(lc_dst_std_name) == 'cloud_liquid_dry_mixing_ratio') then
// CHECK-NEXT:          lc_found = .true.
// CHECK-NEXT:          call lc_tmp(lc_j)%units(lc_dst_units)
// CHECK-NEXT:          if (trim(lc_dst_units) /= 'kg kg-1') then
...
// CHECK-NEXT:      do lc_j = 1, lc_num
// CHECK-NEXT:        call lc_tmp(lc_j)%standard_name(lc_dst_std_name)
// CHECK-NEXT:        if (trim(lc_dst_std_name) == 'cloud_ice_dry_mixing_ratio') then
// CHECK-NEXT:          lc_found = .true.
// CHECK-NEXT:          call lc_tmp(lc_j)%units(lc_dst_units)
// CHECK-NEXT:          if (trim(lc_dst_units) /= 'kg kg-1') then
// CHECK-NEXT:            write(errmsg, '(3a)') 'ccp_model_const_add_metadata ERROR: Trying to add constituent ', &
...
// CHECK-NEXT:        if (trim(lc_std_name) == 'cloud_liquid_dry_mixing_ratio') then
