...
//
// RUN: python3 tests/filecheck/examples/end_to_end/kw_override_suite.py --legacy-mode | python3 -m xdsl_ccpp.tools.ccpp_opt -p generate-meta-cap,generate-meta-kinds,generate-arg-ownership,generate-suite-cap,generate-ccpp-cap,generate-cpp-cap,generate-kinds,strip-ccpp -t ftn | python3 -m filecheck %s
...
// CHECK-NEXT:    public :: kw_suite_register
// CHECK-NEXT:    public :: kw_suite_initialize
// CHECK-NEXT:    public :: kw_suite_finalize
// CHECK-NEXT:    public :: kw_suite_init_physics
// CHECK-NEXT:    public :: kw_suite_physics
// CHECK-NEXT:    public :: kw_suite_timestep_init_physics
// CHECK-NEXT:    public :: kw_suite_timestep_final_physics
// CHECK-NEXT:    public :: kw_suite_final_physics
// CHECK:       CONTAINS
// CHECK-LABEL:   subroutine kw_suite_register(errflg, errmsg)
// CHECK:           integer, intent(out) :: errflg
// CHECK-NEXT:      character(len=512), intent(out) :: errmsg
// CHECK:           errflg = 0
// CHECK-NEXT:      errmsg = ''
// CHECK-NEXT:    end subroutine kw_suite_register
// CHECK-LABEL:   subroutine kw_suite_initialize(errflg, errmsg)
// CHECK:           integer, intent(out) :: errflg
// CHECK-NEXT:      character(len=512), intent(out) :: errmsg
// CHECK:           errflg = 0
// CHECK-NEXT:      errmsg = ''
...
// CHECK-NEXT:    end subroutine kw_suite_initialize
// CHECK-LABEL:   subroutine kw_suite_finalize(errflg, errmsg)
// CHECK:           integer, intent(out) :: errflg
// CHECK-NEXT:      character(len=512), intent(out) :: errmsg
// CHECK:           errflg = 0
// CHECK-NEXT:      errmsg = ''
...
// CHECK-NEXT:    end subroutine kw_suite_finalize
// CHECK-LABEL:   subroutine kw_suite_init_physics(errmsg, errflg)
...
// CHECK-NEXT:      if (.NOT. (const_initialized .eq. ccpp_suite_state)) then
// CHECK-NEXT:        write(errmsg, '(3a)') "Invalid initial CCPP state, '", trim(ccpp_suite_state),              &
// CHECK-NEXT:          "' in kw_suite_init_physics"
// CHECK-NEXT:        errflg = 1
// CHECK-NEXT:      end if
// CHECK-NEXT:      if (errflg .eq. 0) then
// CHECK-NEXT:        call hello_scheme_init(errmsg=errmsg, errflg=errflg)
// CHECK-NEXT:      end if
// CHECK-NEXT:    end subroutine kw_suite_init_physics
// CHECK-LABEL:   subroutine kw_suite_physics(col_start, col_end, lev, errmsg, errflg)
...
// CHECK-NEXT:      end if
// CHECK-NEXT:    end subroutine kw_suite_physics
// CHECK-LABEL:   subroutine kw_suite_timestep_init_physics(errflg, errmsg)
// CHECK:           integer, intent(out) :: errflg
// CHECK-NEXT:      character(len=512), intent(out) :: errmsg
// CHECK:           errflg = 0
// CHECK-NEXT:      errmsg = ''
// CHECK-NEXT:      ccpp_suite_state = const_in_time_step
// CHECK-NEXT:    end subroutine kw_suite_timestep_init_physics
// CHECK-LABEL:   subroutine kw_suite_timestep_final_physics(errflg, errmsg)
// CHECK:           integer, intent(out) :: errflg
// CHECK-NEXT:      character(len=512), intent(out) :: errmsg
// CHECK:           errflg = 0
// CHECK-NEXT:      errmsg = ''
// CHECK-NEXT:      ccpp_suite_state = const_initialized
// CHECK-NEXT:    end subroutine kw_suite_timestep_final_physics
// CHECK-LABEL:   subroutine kw_suite_final_physics(errmsg, errflg)
// CHECK:           character(len=512), intent(out) :: errmsg
// CHECK-NEXT:      integer, intent(out) :: errflg
// CHECK:           errflg = 0
// CHECK-NEXT:      errmsg = ''
// CHECK-NEXT:      if (.NOT. (const_initialized .eq. ccpp_suite_state)) then
// CHECK-NEXT:        write(errmsg, '(3a)') "Invalid initial CCPP state, '", trim(ccpp_suite_state),              &
// CHECK-NEXT:          "' in kw_suite_final_physics"
// CHECK-NEXT:        errflg = 1
// CHECK-NEXT:      end if
// CHECK-NEXT:      if (errflg .eq. 0) then
// CHECK-NEXT:        call hello_scheme_finalize(errmsg=errmsg, errflg=errflg)
// CHECK-NEXT:      end if
// CHECK-NEXT:    end subroutine kw_suite_final_physics
...
// CHECK-NEXT:    use kw_suite_cap, only: kw_suite_final_physics
// CHECK-NEXT:    use kw_suite_cap, only: kw_suite_finalize
// CHECK-NEXT:    use kw_suite_cap, only: kw_suite_init_physics
// CHECK-NEXT:    use kw_suite_cap, only: kw_suite_initialize
// CHECK-NEXT:    use kw_suite_cap, only: kw_suite_physics
// CHECK-NEXT:    use kw_suite_cap, only: kw_suite_register
// CHECK-NEXT:    use kw_suite_cap, only: kw_suite_timestep_final_physics
// CHECK-NEXT:    use kw_suite_cap, only: kw_suite_timestep_init_physics
// CHECK:         implicit none
// CHECK-NEXT:    private
...
// CHECK-NEXT:    public :: Kw_ccpp_is_scheme_constituent
// CHECK-NEXT:    public :: Kw_ccpp_deallocate_dynamic_constituents
// CHECK-NEXT:    public :: Kw_ccpp_register_constituents
...
// CHECK-NEXT:        call kw_suite_register(errflg, errmsg)
// CHECK-NEXT:      else
...
// CHECK:           errflg = 0
// CHECK-NEXT:      if (trim(suite_name) .eq. 'kw_suite') then
// CHECK-NEXT:        call kw_suite_initialize(errflg, errmsg)
// CHECK-NEXT:      else
// CHECK-NEXT:        write(errmsg, '(3a)') "No suite named ", trim(suite_name), " found"
// CHECK-NEXT:        errflg = 1
// CHECK-NEXT:      end if
...
// CHECK:           errflg = 0
// CHECK-NEXT:      if (trim(suite_name) .eq. 'kw_suite') then
// CHECK-NEXT:        call kw_suite_finalize(errflg, errmsg)
// CHECK-NEXT:      else
// CHECK-NEXT:        write(errmsg, '(3a)') "No suite named ", trim(suite_name), " found"
// CHECK-NEXT:        errflg = 1
// CHECK-NEXT:      end if
...
// CHECK-NEXT:          call kw_suite_physics(col_start, col_end, lc_lev, errmsg, errflg)
// CHECK-NEXT:        else
...
// CHECK-NEXT:      if (trim(suite_name) .eq. 'kw_suite') then
// CHECK-NEXT:        if (trim(suite_part) .eq. 'physics') then
// CHECK-NEXT:          call kw_suite_timestep_init_physics(errflg, errmsg)
// CHECK-NEXT:        else
// CHECK-NEXT:          write(errmsg, '(3a)') "No suite part named ", trim(suite_part), " found in suite kw_suite"
// CHECK-NEXT:          errflg = 1
// CHECK-NEXT:        end if
...
// CHECK-NEXT:      if (trim(suite_name) .eq. 'kw_suite') then
// CHECK-NEXT:        if (trim(suite_part) .eq. 'physics') then
// CHECK-NEXT:          call kw_suite_timestep_final_physics(errflg, errmsg)
// CHECK-NEXT:        else
// CHECK-NEXT:          write(errmsg, '(3a)') "No suite part named ", trim(suite_part), " found in suite kw_suite"
// CHECK-NEXT:          errflg = 1
// CHECK-NEXT:        end if
...
// CHECK-NEXT:      if (trim(suite_name) .eq. 'kw_suite') then
// CHECK-NEXT:        if (trim(suite_part) .eq. 'physics') then
// CHECK-NEXT:          call kw_suite_init_physics(errmsg, errflg)
// CHECK-NEXT:        else
// CHECK-NEXT:          write(errmsg, '(3a)') "No suite part named ", trim(suite_part), " found in suite kw_suite"
// CHECK-NEXT:          errflg = 1
// CHECK-NEXT:        end if
...
// CHECK-NEXT:      if (trim(suite_name) .eq. 'kw_suite') then
// CHECK-NEXT:        if (trim(suite_part) .eq. 'physics') then
// CHECK-NEXT:          call kw_suite_final_physics(errmsg, errflg)
// CHECK-NEXT:        else
// CHECK-NEXT:          write(errmsg, '(3a)') "No suite part named ", trim(suite_part), " found in suite kw_suite"
// CHECK-NEXT:          errflg = 1
// CHECK-NEXT:        end if
...
// CHECK-NEXT:          allocate(var_list(1))
// CHECK-NEXT:          var_list(1) = 'vertical_layer_dimension            '
...
// CHECK-NEXT:      subroutine Kw_ccpp_is_scheme_constituent(std_name, is_const, errflg, errmsg)
...
// CHECK-NEXT:      end subroutine Kw_ccpp_is_scheme_constituent
// CHECK:           subroutine Kw_ccpp_deallocate_dynamic_constituents()
...
// CHECK-NEXT:      end subroutine Kw_ccpp_deallocate_dynamic_constituents
// CHECK:           subroutine Kw_ccpp_register_constituents(host_constituents, errmsg, errcode)
...
// CHECK-NEXT:      end subroutine Kw_ccpp_register_constituents
// CHECK:           subroutine Kw_ccpp_number_constituents(num_advected, errmsg, errcode, advected)
...
// CHECK:           subroutine Kw_ccpp_initialize_constituents(ncols, pver, errflg, errmsg)
...
// CHECK-NEXT:      end subroutine Kw_ccpp_initialize_constituents
// CHECK:           function Kw_constituents_array() result(ptr)
...
// CHECK:           subroutine Kw_const_get_index(std_name, index, errflg, errmsg)
// CHECK-NEXT:        character(len=*), intent(in) :: std_name
...
// CHECK-NEXT:      end subroutine Kw_const_get_index
// CHECK:           function Kw_model_const_properties() result(ptr)
// CHECK-NEXT:        type(ccpp_constituent_prop_ptr_t), pointer :: ptr(:)
// CHECK-NEXT:        ptr => lc_const_props
// CHECK-NEXT:      end function Kw_model_const_properties
// CHECK:  end module Kw_ccpp_cap
