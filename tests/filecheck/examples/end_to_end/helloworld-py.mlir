...
//
// RUN: python3 examples/helloworld/helloworld_py.py | python3 -m xdsl_ccpp.tools.ccpp_opt -p generate-meta-cap,generate-meta-kinds,generate-host-match,generate-arg-ownership,generate-suite-cap,generate-ccpp-cap,generate-cpp-cap,generate-kinds,strip-ccpp -t ftn | python3 -m filecheck %s
...
// CHECK-NEXT:    public :: hello_world_suite_register
// CHECK-NEXT:    public :: hello_world_suite_initialize
// CHECK-NEXT:    public :: hello_world_suite_finalize
// CHECK-NEXT:    public :: hello_world_suite_init_physics
// CHECK-NEXT:    public :: hello_world_suite_physics
// CHECK-NEXT:    public :: hello_world_suite_timestep_init_physics
// CHECK-NEXT:    public :: hello_world_suite_timestep_final_physics
// CHECK-NEXT:    public :: hello_world_suite_final_physics
// CHECK:       CONTAINS
// CHECK-LABEL:   subroutine hello_world_suite_register(errflg, errmsg)
// CHECK:           integer, intent(out) :: errflg
// CHECK-NEXT:      character(len=512), intent(out) :: errmsg
// CHECK:           errflg = 0
// CHECK-NEXT:      errmsg = ''
// CHECK-NEXT:    end subroutine hello_world_suite_register
// CHECK-LABEL:   subroutine hello_world_suite_initialize(errflg, errmsg)
// CHECK:           integer, intent(out) :: errflg
// CHECK-NEXT:      character(len=512), intent(out) :: errmsg
// CHECK:           errflg = 0
// CHECK-NEXT:      errmsg = ''
...
// CHECK-NEXT:    end subroutine hello_world_suite_initialize
// CHECK-LABEL:   subroutine hello_world_suite_finalize(errflg, errmsg)
// CHECK:           integer, intent(out) :: errflg
// CHECK-NEXT:      character(len=512), intent(out) :: errmsg
// CHECK:           errflg = 0
// CHECK-NEXT:      errmsg = ''
...
// CHECK-NEXT:    end subroutine hello_world_suite_finalize
// CHECK-LABEL:   subroutine hello_world_suite_init_physics(errmsg, errflg)
...
// CHECK-NEXT:      end if
// CHECK-NEXT:    end subroutine hello_world_suite_init_physics
// CHECK-LABEL:   subroutine hello_world_suite_physics(ncol, lev, ilev, timestep, temp_level, temp_layer, errmsg, &
// CHECK:           errflg)
...
// CHECK-NEXT:    end subroutine hello_world_suite_physics
// CHECK-LABEL:   subroutine hello_world_suite_timestep_init_physics(errflg, errmsg)
// CHECK:           integer, intent(out) :: errflg
// CHECK-NEXT:      character(len=512), intent(out) :: errmsg
// CHECK:           errflg = 0
// CHECK-NEXT:      errmsg = ''
// CHECK-NEXT:      ccpp_suite_state = const_in_time_step
// CHECK-NEXT:    end subroutine hello_world_suite_timestep_init_physics
// CHECK-LABEL:   subroutine hello_world_suite_timestep_final_physics(errflg, errmsg)
// CHECK:           integer, intent(out) :: errflg
// CHECK-NEXT:      character(len=512), intent(out) :: errmsg
// CHECK:           errflg = 0
// CHECK-NEXT:      errmsg = ''
// CHECK-NEXT:      ccpp_suite_state = const_initialized
// CHECK-NEXT:    end subroutine hello_world_suite_timestep_final_physics
// CHECK-LABEL:   subroutine hello_world_suite_final_physics(errmsg, errflg)
// CHECK:           character(len=512), intent(out) :: errmsg
// CHECK-NEXT:      integer, intent(out) :: errflg
// CHECK:           errflg = 0
// CHECK-NEXT:      errmsg = ''
...
// CHECK-NEXT:      end if
// CHECK-NEXT:    end subroutine hello_world_suite_final_physics
...
// CHECK-NEXT:    use hello_world_suite_cap, only: hello_world_suite_final_physics
// CHECK-NEXT:    use hello_world_suite_cap, only: hello_world_suite_finalize
// CHECK-NEXT:    use hello_world_suite_cap, only: hello_world_suite_init_physics
// CHECK-NEXT:    use hello_world_suite_cap, only: hello_world_suite_initialize
// CHECK-NEXT:    use hello_world_suite_cap, only: hello_world_suite_physics
// CHECK-NEXT:    use hello_world_suite_cap, only: hello_world_suite_register
// CHECK-NEXT:    use hello_world_suite_cap, only: hello_world_suite_timestep_final_physics
// CHECK-NEXT:    use hello_world_suite_cap, only: hello_world_suite_timestep_init_physics
// CHECK:         implicit none
// CHECK-NEXT:    private
...
// CHECK-NEXT:        call hello_world_suite_register(errflg, errmsg)
// CHECK-NEXT:      else
...
// CHECK:           errflg = 0
// CHECK-NEXT:      if (trim(suite_name) .eq. 'hello_world_suite') then
// CHECK-NEXT:        call hello_world_suite_initialize(errflg, errmsg)
// CHECK-NEXT:      else
// CHECK-NEXT:        write(errmsg, '(3a)') "No suite named ", trim(suite_name), " found"
// CHECK-NEXT:        errflg = 1
// CHECK-NEXT:      end if
...
// CHECK:           errflg = 0
// CHECK-NEXT:      if (trim(suite_name) .eq. 'hello_world_suite') then
// CHECK-NEXT:        call hello_world_suite_finalize(errflg, errmsg)
// CHECK-NEXT:      else
// CHECK-NEXT:        write(errmsg, '(3a)') "No suite named ", trim(suite_name), " found"
// CHECK-NEXT:        errflg = 1
// CHECK-NEXT:      end if
...
// CHECK-NEXT:          call hello_world_suite_physics(ncol, pver, pverp, dt,                                     &
...
// CHECK-NEXT:      if (trim(suite_name) .eq. 'hello_world_suite') then
// CHECK-NEXT:        if (trim(suite_part) .eq. 'physics') then
// CHECK-NEXT:          call hello_world_suite_timestep_init_physics(errflg, errmsg)
// CHECK-NEXT:        else
// CHECK-NEXT:          write(errmsg, '(3a)') "No suite part named ", trim(suite_part),                           &
// CHECK-NEXT:            " found in suite hello_world_suite"
// CHECK-NEXT:          errflg = 1
...
// CHECK-NEXT:      if (trim(suite_name) .eq. 'hello_world_suite') then
// CHECK-NEXT:        if (trim(suite_part) .eq. 'physics') then
// CHECK-NEXT:          call hello_world_suite_timestep_final_physics(errflg, errmsg)
// CHECK-NEXT:        else
// CHECK-NEXT:          write(errmsg, '(3a)') "No suite part named ", trim(suite_part),                           &
// CHECK-NEXT:            " found in suite hello_world_suite"
// CHECK-NEXT:          errflg = 1
...
// CHECK-NEXT:      if (trim(suite_name) .eq. 'hello_world_suite') then
// CHECK-NEXT:        if (trim(suite_part) .eq. 'physics') then
// CHECK-NEXT:          call hello_world_suite_init_physics(errmsg, errflg)
// CHECK-NEXT:        else
// CHECK-NEXT:          write(errmsg, '(3a)') "No suite part named ", trim(suite_part),                           &
// CHECK-NEXT:            " found in suite hello_world_suite"
// CHECK-NEXT:          errflg = 1
...
// CHECK-NEXT:      if (trim(suite_name) .eq. 'hello_world_suite') then
// CHECK-NEXT:        if (trim(suite_part) .eq. 'physics') then
// CHECK-NEXT:          call hello_world_suite_final_physics(errmsg, errflg)
// CHECK-NEXT:        else
// CHECK-NEXT:          write(errmsg, '(3a)') "No suite part named ", trim(suite_part),                           &
// CHECK-NEXT:            " found in suite hello_world_suite"
// CHECK-NEXT:          errflg = 1
