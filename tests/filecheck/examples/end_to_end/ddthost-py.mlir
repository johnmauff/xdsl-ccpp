...
//
// RUN: python3 examples/ddthost/scheme/ddthost_py.py | python3 -m xdsl_ccpp.tools.ccpp_opt -p generate-meta-cap,generate-meta-kinds,generate-arg-ownership,generate-suite-cap,generate-ccpp-cap,generate-cpp-cap,generate-kinds,strip-ccpp -t ftn | python3 -m filecheck %s
...
// CHECK-LABEL:   subroutine ddt_suite_init_data_prep(nbox, ccpp_info, o3, hno3, model_times, vmr, errmsg,        &
// CHECK:           errflg, ntimes)
...
// CHECK-NEXT:      integer, target, intent(inout) :: model_times(:)
...
// CHECK-LABEL:   subroutine ddt_suite_data_prep(cols, cole, O3, HNO3, vmr, psurf, errmsg, errflg)
...
// CHECK-LABEL:   subroutine ddt_suite_timestep_final_data_prep(ncols, vmr, errmsg, errflg)
...
// CHECK-NEXT:    public :: Ddt_advected_constituents_array
...
// CHECK-NEXT:          call ddt_suite_data_prep(lc_cols, lc_cole, lc_O3, lc_HNO3, lc_vmr, lc_psurf, errmsg,      &
// CHECK-NEXT:            errflg)
// CHECK-NEXT:        else
...
// CHECK-NEXT:      if (trim(suite_name) .eq. 'ddt_suite') then
// CHECK-NEXT:        if (trim(suite_part) .eq. 'data_prep') then
// CHECK-NEXT:          call ddt_suite_timestep_final_data_prep(ncols, lc_vmr, errmsg, errflg)
// CHECK-NEXT:        else
// CHECK-NEXT:          write(errmsg, '(3a)') "No suite part named ", trim(suite_part), " found in suite ddt_suite"
// CHECK-NEXT:          errflg = 1
// CHECK-NEXT:        end if
...
// CHECK-LABEL:   subroutine ccpp_physics_init(suite_name, suite_part, nbox, ccpp_info, model_times, errmsg,      &
// CHECK:           errflg)
...
// CHECK-NEXT:      if (trim(suite_name) .eq. 'ddt_suite') then
// CHECK-NEXT:        if (trim(suite_part) .eq. 'data_prep') then
// CHECK-NEXT:          call ddt_suite_init_data_prep(nbox, ccpp_info, lc_O3, lc_HNO3, model_times, ccpp_tmp_0,   &
// CHECK-NEXT:            errmsg, errflg, ccpp_tmp_1)
// CHECK-NEXT:        else
// CHECK-NEXT:          write(errmsg, '(3a)') "No suite part named ", trim(suite_part), " found in suite ddt_suite"
// CHECK-NEXT:          errflg = 1
// CHECK-NEXT:        end if
...
// CHECK-LABEL:   subroutine ccpp_physics_final(suite_name, suite_part, ntimes, model_times, errmsg, errflg)
// CHECK:           character(len=*), intent(in) :: suite_name
// CHECK-NEXT:      character(len=*), intent(in) :: suite_part
...
// CHECK-NEXT:      if (trim(suite_name) .eq. 'ddt_suite') then
// CHECK-NEXT:        if (trim(suite_part) .eq. 'data_prep') then
// CHECK-NEXT:          call ddt_suite_final_data_prep(ntimes, model_times, errmsg, errflg)
// CHECK-NEXT:        else
// CHECK-NEXT:          write(errmsg, '(3a)') "No suite part named ", trim(suite_part), " found in suite ddt_suite"
// CHECK-NEXT:          errflg = 1
// CHECK-NEXT:        end if
...
// CHECK-LABEL:   subroutine Ddt_ccpp_register_constituents(host_constituents, errmsg, errcode)
...
// CHECK-LABEL:   subroutine Ddt_ccpp_number_constituents(num_advected, errmsg, errcode, advected)
...
// CHECK-NEXT:    function Ddt_constituents_array() result(ptr)
...
// CHECK-NEXT:    function Ddt_advected_constituents_array() result(ptr)
// CHECK-NEXT:      real(kind=kind_phys), pointer :: ptr(:, :, :)
// CHECK-NEXT:      ptr => cam_constituents_obj%advected_constituents_ptr()
// CHECK-NEXT:    end function Ddt_advected_constituents_array
...
// CHECK-NEXT:    function Ddt_model_const_properties() result(ptr)
