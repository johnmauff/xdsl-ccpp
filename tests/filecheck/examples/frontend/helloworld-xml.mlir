...
//
// RUN: python3 -m xdsl_ccpp.frontend.ccpp_xml --suites examples/helloworld/hello_world_suite.xml --scheme-files examples/helloworld/hello_scheme.meta,examples/helloworld/temp_adjust.meta --host-files examples/helloworld/hello_world_host.meta,examples/helloworld/hello_world_mod.meta | python3 -m filecheck %s
...
// CHECK-NEXT:      }) : () -> ()
// CHECK-NEXT:    }) {source_module = "hello_world_host"} : () -> ()
...
// CHECK-NEXT:      }) : () -> ()
// CHECK-NEXT:    }) {source_module = "hello_world_mod"} : () -> ()
// CHECK-NEXT:  }
