...
// CHECK-NEXT:        "ccpp.arg"() <{name = "effrr_in", type = "real", dimensions = #builtin.int<2>, dim_names = "horizontal_dimension,vertical_layer_dimension", standard_name = "effective_radius_of_stratiform_cloud_rain_particle", long_name = "effective radius of cloud rain particle in micrometer", kind = "kind_phys", intent = "in", units = "um", top_at_one}> : () -> ()
...
// CHECK-NEXT:        "ccpp.arg"() <{name = "effrs_inout", type = "real", dimensions = #builtin.int<2>, dim_names = "horizontal_dimension,vertical_layer_dimension", standard_name = "effective_radius_of_stratiform_cloud_snow_particle", long_name = "effective radius of cloud snow particle in micrometer", kind = "8", intent = "inout", units = "um", top_at_one}> : () -> ()
...
// CHECK-NEXT:        "ccpp.arg"() <{name = "errflg", type = "integer", standard_name = "ccpp_error_code", long_name = "Error flag for error handling in CCPP", intent = "out", units = "1"}> : () -> ()
// CHECK-NEXT:      }) : () -> ()
// CHECK-NEXT:    }) {source_module = "rad_lw", dependencies_path = "{{.*}}", dependencies = ["module_rad_ddt.F90"]} : () -> ()
...
// CHECK-NEXT:      }) : () -> ()
// CHECK-NEXT:    }) {source_module = "test_host_data", dependencies_path = "{{.*}}", dependencies = ["module_rad_ddt.F90"]} : () -> ()
